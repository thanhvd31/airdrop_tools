...
class Birdton {
    ...
    async fetchAds(userId, proxy) {
        const url = `https://api.adsgram.ai/adv?blockId=604&tg_id=${userId}&tg_platform=android&platform=Win32&language=vi`;
        const agent = new HttpsProxyAgent(proxy);
        try {
            const response = await axios.get(url, { headers: this.headers, httpAgent: agent, httpsAgent: agent });
            ...
            await axios.get(renderUrl, { headers: this.headers, httpAgent: agent, httpsAgent: agent });
            await axios.get(showUrl, { headers: this.headers, httpAgent: agent, httpsAgent: agent });
            await axios.get(rewardUrl, { headers: this.headers, httpAgent: agent, httpsAgent: agent });
    ...
    connectWebSocket(auth_key, payload, balance, proxy) {
            ...
            try {
                ...
                if (parsedMessage.event_type === 'boost') {
                    if (!this.adsProcessed && this.ads_left > 0) {
                        await this.handleAds(this.userId, proxy);
    ...
    async auth(payload, userData, proxy) {
        try {
            const url = `${this.baseURL}/auth`;
            const agent = new HttpsProxyAgent(proxy);
            const response = await axios.post(url, payload, { headers: this.headers, httpAgent: agent, httpsAgent: agent });
            const data = response.data;
            ...
            if (energy > 0) {
                this.connectWebSocket(auth_key, payload, balance, proxy);
    ...
    async handleAds(userId, proxy) {
        for (let i = 0; i < this.initialAdsLeft; i++) {
            await this.fetchAds(userId, proxy);
        }
    }
    ...
    async main() {
        ...
        while (true) {
            for (let [index, token] of tokens.entries()) {
                ...
                console.log(`========== Tài khoản ${index + 1}/${tokens.length} | ${firstName} | IP: ${proxyIP} ==========`);
                ...
                await this.auth(payload, userData, proxy);
